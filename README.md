...
- **石墨文档**：应用内嵌网页直接接入石墨文档（个人版没有开放 API，内嵌网页即接入面）——在「设置 → 石墨文档」填上你的表格链接，「石墨文档」标签页即在应用内打开它，扫码/手机号登录一次后长期有效，可像在浏览器里一样查看和管理自己的表格；只允许访问石墨域名，页面里的站外链接自动转系统浏览器打开
...
- **石墨文档**：填写要管理的那张石墨表格链接（浏览器地址栏复制，形如 `https://shimo.im/sheets/…`）；留空则「石墨文档」标签页只显示配置引导。表格加载后在内嵌网页里登录你自己的石墨账号即可，登录态只保存在本机

### 石墨文档（内嵌网页）

个人版石墨没有开放 API，因此应用以 Electron `<webview>` 内嵌 shimo.im 网页本体作为接入方式：

- **登录**：第一次打开「石墨文档」标签时，在内嵌网页里用你的石墨账号登录（手机号+验证码 / 扫码），登录态保存在专用分区（`persist:shimo`），重启应用仍然有效；与搜索用的闲鱼/淘宝登录相互隔离
- **页面**：默认加载设置里配置的表格链接；工具条提供后退/前进/刷新/「石墨工作台」/地址栏/「在系统浏览器中打开」/「退出登录」（清除本机登录态）
- **域名锁定**：内嵌网页只允许访问石墨域名（shimo.im）；页面中点击的站外链接会自动转到系统浏览器打开，非导航类协议（`javascript:` 等）直接拦截
- **刷新**：切走再切回「石墨文档」标签会重新加载页面，从石墨服务器拿到最新内容
- **注意**：钉钉/企业微信等第三方登录会跳出石墨域名，会被转去系统浏览器、登录态不会同步到应用内，请使用手机号+验证码或石墨扫码登录
