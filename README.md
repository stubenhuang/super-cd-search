...
- **局域网连接（LAN Connection）**：点击首页顶部的手机图标打开独立面板；启用后在本机启动一个只绑定局域网 IPv4 地址的 HTTP 服务（公网 IP 会被拒绝），并提供含随机访问令牌的二维码。手机扫码即可打开连接页面，可拍照或手动输入 CD 条码，通过供应商链反查目录号并加入桌面搜索框。条码解析供应商的顺序/启停仍在「设置 → 条码解析供应商」中配置
